...
def main():
    ...
    # Inject some realistic experimental noise
    np.random.seed(42)
    noise = 0.02 * np.max(F_data) * np.random.randn(len(F_data))
    F_data_noisy = F_data + noise
    
    # 2. Run the Identification Pipeline (with smoothing!)
    print("Running Laplace -> Time-Domain optimization with noisy data...")
    S_VALS = np.sort(np.unique(np.logspace(-3, 3, 110)))
    
    results = run_fit(
        model_name="Standard_Linear_Solid",
        t_full=t_arr, h_full=h_arr, F_full=F_data_noisy,
        C=C, h_power_exp=h_power_exp,
        S_VALS=S_VALS,
        time_window=5.0,       
        refine_full_data=True,
        smooth_window=15        # Analytical SG derivative will be used
    )
    ...
    out_dir = os.path.join(os.path.dirname(__file__), "basic_fit2 output")
    save_results(results, "Standard_Linear_Solid", t_arr, F_data_noisy, output_dir=out_dir)
    plot_comparison(results, t_arr, F_data_noisy, "Standard_Linear_Solid", output_dir=out_dir)
    
    print("\nExample complete! Check the 'basic_fit2 output' folder.")
